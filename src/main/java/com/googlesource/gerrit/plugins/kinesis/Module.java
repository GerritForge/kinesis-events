...
public class Module extends LifecycleModule {
  ...
  /**
   * By default the multi-site plugin registers a noop implementation of the events-broker, which
   * itself registers a list of topic subscribers. Since we have no control over which plugin is
   * loaded first (multi-site or kinesis-events), we deal with the possibility that a broker api was
   * already registered and, in that case, reassign its consumers to the kinesis-events plugin. The
   * injection is optional because if the kinesis-events plugin is loaded first, then there will be
   * no previously registered broker API.
   *
