...
class KinesisConfiguration {
  ...
  public KinesisConfiguration(PluginConfigFactory configFactory, @PluginName String pluginName) {
    ...
    Region region = Region.of(getStringParam(pluginConfig, "region", DEFAULT_REGION));
    Optional<URI> endpoint =
        Optional.ofNullable(getStringParam(pluginConfig, "endpoint", null)).map(URI::create);
    this.streamEventsTopic = getStringParam(pluginConfig, "topic", "gerrit");
    ...
    // TODO: get initial position from system prop
    this.initialPosition =
        pluginConfig.getEnum(
            InitialPositionInStream.values(), "initialPosition", InitialPositionInStream.LATEST);
    ...
    logger.atInfo().log(
        "Kinesis client. Application:'%s'|region:'%s'.%s",
        applicationName,
        region,
  ...
  public RetrievalConfig getRetrievalConfig(
    ...
    PollingConfig polling =
        new PollingConfig(streamName, kinesisClient)
            .idleTimeBetweenReadsInMillis(1000L)
            .maxRecords(10000);
